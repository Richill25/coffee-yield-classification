---
title: "The Business Challenge: Smallholder Coffee Farmer Income Optimization in East Africa"
author: "Richill Ataa Nyantakyiwaa"
date: "2025-12-04"
output:
  pdf_document:
...
hist(yields, breaks = 50, col = rgb(0.2, 0.4, 0.8, alpha = 0.7),
     ...
     main = "Distribution of Coffee Yields with Classification Thresholds")
...
ntrees <- c(10, 50, 100, 200, 500, 1000)
oob_errors <- numeric(length(ntrees))

for(i in 1:length(ntrees)) {
  rf_temp <- randomForest(yield_class ~ ., data = coffee_data,
                          ntree = ntrees[i],
                          mtry = sqrt(p),
                          nodesize = 5)
  oob_errors[i] <- rf_temp$err.rate[ntrees[i], 1]
}

plot(ntrees, oob_errors, type = "b", col = "blue", lwd = 2,
     xlab = "Number of Trees",
     ylab = "OOB Error",
     main = "OOB Error vs Number of Trees")

optimal_ntree <- ntrees[which.min(oob_errors)]
...
I train a Random Forest with 100 trees using the `randomForest` package:
...
cat("Random Forest Training Error:", rf_model$err.rate[100, 1], "\n")
...
cat("Random Forest OOB Error:", rf_model$err.rate[100, 1], "\n")
...
rf_final <- randomForest(yield_class ~ ., data = train_data, 
                         ntree = 100, mtry = sqrt(ncol(train_data)), 
...
cat("Interpretation: We are 95% confident the true accuracy lies between", round(ci_lower * 100, 2), "% and", round(ci_upper * 100, 2), "%\n")
```
